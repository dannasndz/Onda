...
const PLACEHOLDER_IMAGE = '/images/placeholder-music.png';
...
export async function GET(req: Request) {
    ...
    try {
        const lastfmPage = page;
        const lastfmLimit = limit;
        ...
        const searchUrl = `${BASE_URL}?method=${method}&${entityParam}=${encodeURIComponent(q)}&page=${lastfmPage}&limit=${lastfmLimit}&api_key=${API_KEY}&format=json`;
        ...
        try {
            ...
            let totalResults = 0;
            if (data.results && data.results['opensearch:totalResults']) {
            ...
            const seen = new Set();
            const uniqueResultados = resultados.filter(item => {
            ...
            if (fetchError instanceof Error && fetchError.name === 'AbortError') {
                return NextResponse.json({ error: 'La solicitud a Last.fm tardó demasiado tiempo' }, { status: 408 });
            }
            throw fetchError;
        }
    }
