...
export async function GET(req: Request) {
    ...
    try {
        ...
        const resultadosCrudos = await Promise.all(
            items.map(async (item: any) => {
                ...
                if (isDefaultImage(imagen)) {
                    ...
                    usaPlaceholder = !itunesCover;
                }

                resultado.imagen = imagen;
                resultado.usaPlaceholder = usaPlaceholder;
        ...
        const resultados = resultadosCrudos.filter((item) => {
            const claveUnica =
                tipo === 'artista'
                    ? item.nombre.toLowerCase()
                    : `${item.nombre.toLowerCase()}-${item.artista.toLowerCase()}`;
